  ...
  async function syncPendingToFirestore() {
    try {
      ...
      for (const act of pending) {
        try {
          // Use a transaction for read-modify-write operations (updates/deletes)
          await db.runTransaction(async (transaction) => {
            let userDoc; // To store doc snapshot if needed

            switch (act.type) {
              case 'addSale':
                // Simple append, no read needed
                transaction.update(userRef, { sales: FieldValue.arrayUnion(act.item) });
                break;
                
              case 'removeSale':
                // Simple removal
                transaction.update(userRef, { sales: FieldValue.arrayRemove(act.item) });
                break;
                
              case 'addProduct':
                // Simple append
                transaction.update(userRef, { products: FieldValue.arrayUnion(act.item) });
                break;
                
              case 'removeProduct':
                // Simple removal
                transaction.update(userRef, { products: FieldValue.arrayRemove(act.item) });
                break;
                
              case 'updateProduct':
                // This is a Read-Modify-Write operation, must use transaction.get
              ...
              case 'addStock':
                // This is also a Read-Modify-Write
                userDoc = await transaction.get(userRef);
                if (!userDoc.exists) throw new Error("User document not found");
          ...
          // If transaction was successful, clear the item from the queue
