...
* Author: Gemini
* Date: 2025-11-15
* Summary: Implemented Req 2 (Report scroll) & Req 3 (Panel scroll reset).
* - Req 2: Removed `scrollIntoView` call from `renderReports` to prevent auto-scrolling to the chart.
* - Req 3: Modified `setActiveView` to set `scrollTop = 0` on the newly activated panel, ensuring all panels open to the top.
*/
...
function hideAllAuthForms() {
  ...
  // saveState: Only saves notes to cloud. Saves everything to local.
  async function saveState() {
    ...
    try {
      // Only sync non-conflicting, non-queued data (like notes)
      const notesData = {
        notes: state.notes || [],
        lastSync: Date.now()
      };
      await getDb().collection('users').doc(currentUser.uid).set(notesData, { merge: true });
    } catch (e) {
      errlog('saveState (notes sync) failed', e);
      toast('Cloud sync for notes failed.', 'error');
    } finally {
      isSyncing = false;
    }
  }
  
  // loadLocalData: Runs immediately, loads from localStorage, renders UI
  function loadLocalData(uid = null) {
    const localKey = uid ? LOCAL_KEY_PREFIX + uid : LOCAL_KEY_PREFIX + 'anon';
    let localState = { products: [], sales: [], changes: [], notes: [] };
  ...
  // syncCloudData: Runs after auth, merges cloud data, re-renders
  ...
  if (addProductBtn) addProductBtn.addEventListener('click', async function () {
    ...
    if (!valid.valid) { 
      const modal = addProductBtn.closest('.modal');
      if (modal) {
  ...
  async function removeProduct(id) {
    const p = state.products.find(x => x.id === id);
    if (!p) return;
    if (!confirm(`Delete "${p.name}"? This will remove it from inventory and sync the deletion.`)) return;
    ...
    await saveState(); 
    renderInventory(); renderProducts(); renderDashboard(); renderCustomList(); renderNotes();
  ...
  function renderDashboard() {
    ...
    // **AI FIX**: This now calculates top seller based on *all* sales, not just today
    ...
    let topName = '—';
    if (overallArr.length && overallArr[0][0]) {
        const topProd = state.products.find(p => p.id === overallArr[0][0]);
  ...
  if (btnLoadDemo) btnLoadDemo.addEventListener('click', async function () {
    ...
    await saveState(); 
    renderInventory(); renderProducts(); renderDashboard(); renderCustomList(); toast('Demo loaded');
  });
  if (btnClearStore) btnClearStore.addEventListener('click', async function () {
    ...
    await saveState(); 
    renderInventory(); renderProducts(); renderDashboard(); renderCustomList(); renderNotes(); toast('Store cleared');
  });
  ...
  function renderCustomList() {
    ...
    if (!state.products || state.products.length === 0) { 
        const no = document.createElement('div');
        no.className = 'small';
        no.textContent = 'No products.';
        customListArea.appendChild(no);
        return; 
    }
    ...
    for (const p of state.products) {
      ...
      if (p.image) { 
        const im = document.createElement('img'); im.src = p.image; im.style.cssText = 'width:36px;height:36px;object-fit:cover'; im.crossOrigin = 'anonymous'; imgWrap.appendChild(im); 
      } else if (p.icon) { 
        const ic = document.createElement('div'); ic.style.cssText = 'font-size:18px;padding:6px'; ic.innerText = p.icon; imgWrap.appendChild(ic); 
      } else { 
        const ic = document.createElement('div'); ic.style.cssText = 'padding:6px;font-weight:700'; ic.innerText = (p.name||'').slice(0,2).toUpperCase(); imgWrap.appendChild(ic); 
      }
      
      const txt = document.createElement('div'); 
      const strong = document.createElement('strong'); strong.textContent = p.name; 
      const small = document.createElement('div'); small.className = 'small'; small.textContent = `${p.qty || 0} in stock • ${fmt(p.price)}`; 
  ...
  function setActiveView(view) {
    ...
    // [GEMINI] REQ 3: Reset scroll position to top when any panel is opened
    // This timeout ensures the panel is visible before scrolling
    setTimeout(()=> { 
      try { 
        const el = document.querySelector('.panel.active'); 
        if (el) el.scrollTop = 0; 
      } catch(e){} 
    }, 10); // 10ms is enough for the display:block to apply
  }
  ...
  window.__QS_APP = {
    getAuth, getDb, getStorage, saveState, getState: () => state, startScanner, stopScanner, generateInsights, syncCloudData
  };
